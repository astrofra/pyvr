...
def update_controllers(scene, controllers, prg_ref, controller_ref):
	...
	for n in vr_controller_names:
		...
		if controller.IsConnected():
			...
			if "node" not in controllers[n]:
				...
				mat_cube_controller = create_material(hg.Vec4(0, 1, 0, 1), hg.Vec4(1, 0.658, 0., 1), prg_ref)
...
def main():
	...
	win = hg.RenderInit("Harfang - OpenVR Scene Controllers", res_x, res_y, hg.RF_VSync | hg.RF_MSAA4X)
	...
	mat_cube = create_material(hg.Vec4(255 / 255, 230 / 255, 20 / 255, 1), hg.Vec4(1, 0.658, 0., 1), prg_ref)
	hg.CreateObject(scene, hg.TransformationMat4(hg.Vec3(0, 0.5, 0), hg.Vec3(0, hg.Deg(70), 0)), cube_ref, [mat_cube])

	mat_ground = create_material(hg.Vec4(255 / 255, 120 / 255, 147 / 255, 1), hg.Vec4(1, 1, 0.1, 1), prg_ref)
	...
	while not hg.ReadKeyboard().Key(hg.K_Escape):
		...
		controllers = update_controllers(scene, controllers, prg_ref, controller_ref)
